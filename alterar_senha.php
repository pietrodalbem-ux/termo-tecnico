...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $novo_nome_seguranca = trim($_POST['novo_nome_seguranca']); 
    ...
    if ($stmt->execute()) {
        $mensagem = '<div class="alert alert-success py-2 small fw-bold shadow-sm border-0"><i class="bi bi-check-circle me-1"></i> Dados atualizados com sucesso!</div>';
    } else {
        $mensagem = '<div class="alert alert-danger py-2 small fw-bold shadow-sm border-0"><i class="bi bi-x-circle me-1"></i> Erro ao atualizar os dados.</div>';
    }
    $stmt->close();
}
...
<head>
    ...
    <style>
        ...
        [data-bs-theme="light"] input.bg-dark { background-color: #f8f9fa !important; color: #212529 !important; }
        [data-bs-theme="light"] .bg-body-tertiary { background-color: #f8f9fa !important; }
    </style>
</head>
<body class="bg-dark d-flex align-items-center justify-content-center vh-100 p-3 position-relative">
    ...
    <div class="card bg-dark border-warning shadow-lg w-100" style="max-width: 480px;">
        <div class="card-body p-4 p-md-5">
            ...
            <form method="POST">
                <div class="bg-body-tertiary p-3 rounded mb-3 border border-secondary">
                    <label class="form-label text-secondary fw-bold small"><i class="bi bi-key"></i> Nova Senha de Login</label>
                    <input type="password" name="nova_senha" class="form-control bg-dark text-white border-secondary shadow-sm" placeholder="Opcional">
                </div>

                <div class="bg-body-tertiary p-3 rounded mb-4 border border-warning">
                    <label class="form-label text-warning fw-bold small"><i class="bi bi-person-badge"></i> Nome de Segurança</label>
                    ...
                    <div class="form-text text-secondary" style="font-size: 0.75rem;">Utilizado para recuperar acesso caso esqueça a senha.</div>
                </div>
                
                <div class="d-flex gap-2">
                    <a href="admin.php" class="btn btn-outline-secondary w-50 py-2"><i class="bi bi-arrow-left"></i> Voltar</a>
                    <button type="submit" class="btn btn-warning w-50 fw-bold shadow py-2"><i class="bi bi-check2"></i> Salvar</button>
                </div>
            </form>
